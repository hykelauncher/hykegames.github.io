var Stats = function () {
  ...
  ac.style.backgroundColor =
    "rgb(" +
    Math.floor(ak.fps.bg.r / 2) +
    "," +
    Math.floor(ak.fps.bg.g / 2) +
    "," +
    Math.floor(ak.fps.bg.b / 2) +
    ")";
  ...
  ai.style.color =
    "rgb(" + ak.fps.fg.r + "," + ak.fps.fg.g + "," + ak.fps.fg.b + ")";
  ...
  aa = al.getContext("2d");
  aa.fillStyle =
    "rgb(" + ak.fps.bg.r + "," + ak.fps.bg.g + "," + ak.fps.bg.b + ")";
  aa.fillRect(0, 0, al.width, al.height);
  H = aa.getImageData(0, 0, al.width, al.height);
  ah = document.createElement("div");
  ah.style.backgroundColor =
    "rgb(" +
    Math.floor(ak.ms.bg.r / 2) +
    "," +
    Math.floor(ak.ms.bg.g / 2) +
    "," +
    Math.floor(ak.ms.bg.b / 2) +
    ")";
  ...
  aj.style.color =
    "rgb(" + ak.ms.fg.r + "," + ak.ms.fg.g + "," + ak.ms.fg.b + ")";
  ...
  af.style.backgroundColor =
    "rgb(" +
    Math.floor(ak.mb.bg.r / 2) +
    "," +
    Math.floor(ak.mb.bg.g / 2) +
    "," +
    Math.floor(ak.mb.bg.b / 2) +
    ")";
  ...
  ae.style.color =
    "rgb(" + ak.mb.fg.r + "," + ak.mb.fg.g + "," + ak.mb.fg.b + ")";
  ...
  return {
    ...
    update: function () {
      ...
      if (ad > L + 1000) {
        ...
        if (P == 3) {
          (W = performance.memory.usedJSHeapSize * 9.54e-7),
            (S = Math.min(S, W)),
            (Q = Math.max(Q, W)),
            R(O.data, Math.min(30, 30 - W / 2), "mb"),
            (ae.innerHTML =
              '<span style="font-weight:bold">' +
              Math.round(W) +
              " MB</span> (" +
...
(function () {
    ...
    if (z.length > 1 && h.exec(y)) {
      ...
      if (e) {
        F = C
          ...
          : c.find(
              z.pop(),
              z.length === 1 && (z[0] === "~" || z[0] === "+") && e.parentNode
                ? e.parentNode
                : e,
              w
            );
      ...
      if (!x) {
        ...
        if (e && e.nodeType === 1) {
          for (A = 0; J[A] != null; A++) {
            if (
              J[A] &&
              (J[A] === true || (J[A].nodeType === 1 && c.contains(e, J[A])))
            ) {
              B.push(G[A]);
            }
          }
      ...
      "+": function (z, u) {
        ...
        for (var v = 0, e = z.length, x; v < e; v++) {
          if ((x = z[v])) {
            ...
            z[v] =
              A || (x && x.nodeName.toLowerCase() === u) ? x || false : x === u;
          }
        }
        if (A) {
          c.filter(u, z, true);
    ...
    filter: {
      PSEUDO: function (v, A, z, B) {
        ...
        if (u) {
          ...
          if (e === "contains") {
            return (
              (v.textContent || v.innerText || c.getText([v]) || "").indexOf(
                A[3]
              ) >= 0
            );
          } else {
      ...
      TAG: function (u, e) {
        return (
          (e === "*" && u.nodeType === 1) || u.nodeName.toLowerCase() === e
        );
      },
      CLASS: function (u, e) {
        return (
          (" " + (u.className || u.getAttribute("class")) + " ").indexOf(e) > -1
        );
      },
      ATTR: function (y, w) {
        var v = w[1],
  ...
  for (var d in i.match) {
    i.match[d] = new RegExp(
      i.match[d].source + /(?![^\[]*\])(?![^\(]*\))/.source
    );
    i.leftMatch[d] = new RegExp(
  ...
  (function () {
    ...
    if (document.getElementById(v)) {
      ...
      i.filter.ID = function (y, w) {
        var x =
          typeof y.getAttributeNode !== "undefined" && y.getAttributeNode("id");
...
Animator.camelize = function (c) {
  ...
  var b =
    c.indexOf("-") == 0
      ? e[0].charAt(0).toUpperCase() + e[0].substring(1)
      : e[0];
...
ColorStyleSubject.prototype = {
  ...
  setState: function (c) {
    var a =
      "#" +
      ColorStyleSubject.toColorPart(this.getValueForState(0, c)) +
      ColorStyleSubject.toColorPart(this.getValueForState(1, c)) +
      ColorStyleSubject.toColorPart(this.getValueForState(2, c));
...
DiscreteStyleSubject.prototype = {
  setState: function (c) {
    var a = 0;
    for (var b = 0; b < this.els.length; b++) {
      this.els[b].style[this.property] =
        c <= this.threshold ? this.from : this.to;
    }
  },
  inspect: function () {
    return (
...
function CSSStyleSubject(d, m, j) {
  ...
  for (a in b) {
    ...
    if ((l = ColorStyleSubject.parseColor(g))) {
      ...
      if (
        g.match(CSSStyleSubject.numericalRe) &&
        c.match(CSSStyleSubject.numericalRe)
      ) {
        l = parseFloat(g);
        k = parseFloat(c);
        ...
        if (
          g.match(CSSStyleSubject.discreteRe) &&
          c.match(CSSStyleSubject.discreteRe)
        ) {
          l = g;
          k = c;
...
AnimatorChain.prototype = {
  setOptions: function (a) {
    this.options = Animator.applyDefaults({ resetOnPlay: true }, a);
  },
  play: function () {
    this.forwards = true;
    this.current = -1;
...
StateMachine = {
  VERSION: "2.0.0",
  create: function (e, f) {
    var h = typeof e.initial == "string" ? { state: e.initial } : e.initial;
    var d = f || e.target || {};
    var j = e.events || [];
    var g = e.callbacks || {};
    var b = {};
    ...
    if (h) {
      ...
      i({ name: h.event, from: "none", to: h.state });
    }
...
if (!Function.prototype.bind) {
  Function.prototype.bind = function (d) {
    var e = [].slice,
      ...
      c = function () {
        return a.apply(
          this instanceof f ? this : d || {},
          b.concat(e.call(arguments))
        );
      };
  ...
  loadResources: function (i, c, k) {
    ...
    if (h == 0) {
      ...
      for (var a = 0; a < i.length; a++) {
        var d = i[a];
        d = is.string(d) ? { id: d, url: d } : d;
        b.images[d.id] = Game.createImage(d.url, { onload: e });
      }
      for (var a = 0; a < c.length; a++) {
        var j = c[a];
        j = is.string(j) ? { id: j, name: j } : j;
...
Game.Menu = Class.create({
  ...
  setChoice: function (f, g) {
    if (f.cfg.choice) {
      ...
      var c = $({ tag: "span", klass: "choice", text: f.cfg.choice[a] });
      var e = $({ tag: "span", klass: "prev", text: a == 0 ? "" : "<" });
      var d = $({
        tag: "span",
        klass: "next",
        text: a == b.length - 1 ? "" : ">",
  ...
  select: function (c) {
    if (is.valid(c)) {
      var a = this.selectedItem();
      var b = is.number(c)
        ? this.items[Math.min(Math.max(c, 0), this.items.length - 1)]
        : c;
      if (a != b) {
        if (a) {
...
Game.Math = {
  ...
  brighten: function (f, e) {
    ...
    return (
      "#" +
      (0 | ((1 << 8) + d + ((256 - d) * e) / 100)).toString(16).substr(1) +
      (0 | ((1 << 8) + c + ((256 - c) * e) / 100)).toString(16).substr(1) +
      (0 | ((1 << 8) + a + ((256 - a) * e) / 100)).toString(16).substr(1)
    );
  },
  bound: function (a) {
    if (a.radius) {
  ...
  move: function (b, g, d, c, e) {
    var a = d * e;
    var f = c * e;
    return { x: b + a, y: g + f, dx: d, dy: c, nx: a, ny: f };
  },
  accelerate: function (g, e, j, i, c, b) {
    ...
    var h = j + c * b * (j > 0 ? 1 : -1);
    var d = i + c * b * (i > 0 ? 1 : -1);
    return { nx: a - g, ny: f - e, x: a, y: f, dx: h, dy: d };
  },
...
Snakes = function () {
  ...
  CORNER = { NONE: 0, BL: 1, BR: 2, TL: 3, TR: 4 };
  CORNER.LOOKUP = [
  ...
  var a = {
    ...
    difficulties: [
      ...
      { label: "Normal", dstep: 0.07, dscore: 1 },
      { label: "Fast", dstep: 0.05, dscore: 1.25 },
      { label: "Insane", dstep: 0.03, dscore: 1.5 },
      //{ label: "mado", dstep: 0.01, dscore: 0.2 },
    ],
    highscores: [
      { name: "freed", score: 3000 },
      { name: "jake", score: 2500 },
      { name: "michael", score: 2000 },
      { name: "jane", score: 1500 },
      { name: "eva", score: 1000 },
      { name: "joshua", score: 500 },
      { name: "sam", score: 250 },
      { name: "ben", score: 100 },
    ],
    colors: {
    ...
    sparkles: { duration: 1200, spread: 8, max: 200, each: 80 },
  ...
  var b = Class.create({
    ...
    resetDifficulty: function (i) {
      this.storage.difficulty = Game.Math.minmax(
        is.valid(i) ? i : toInt(this.storage.difficulty, 1),
        0,
    ...
    update: function (i) {
      if (this.is("game")) {
        ...
        if (
          ...
          this.playDieFx();
          this.sparkles.add(this.snake.head, { color: a.colors.head });
    ...
    unoccupied: function (o) {
      ...
      while (--i) {
        if (!m.blank()) {
          ...
          j.y = m.y + 1;
          if (
            !this.court.occupies(m) &&
            !this.snake.occupies(m) &&
            !this.court.occupies(n) &&
            !this.snake.occupies(n) &&
            !this.court.occupies(l) &&
            !this.snake.occupies(l) &&
            !this.court.occupies(k) &&
            !this.snake.occupies(k) &&
            !this.court.occupies(j) &&
            !this.snake.occupies(j)
          ) {
            return m;
          }
        }
    ...
    buildQuitMenu: function () {
      return new Game.Menu(this.dom.overlay, this, {
        ...
        items: [
          {
            text: "Quit",
            title: "Quit the current game (Y/N) ?",
            action: function () {
              this.quitGame();
            },
          },
  ...
  var h = Class.create({
    ...
    buildHighScoreTable: function (m, t) {
      ...
      for (j = 0; j < m.length; j++) {
        ...
        k = $({ tag: "span", klass: "score", text: r.score });
        s = $({ tag: "li", klass: q, dom: [k, i] });
        p.append(s);
      }
    ...
    newHighScore: function () {
      var i = { name: this.name, score: this.score };
    ...
    load: function () {
      ...
      this.highscores =
        JSON.parse(this.game.storage.highscores || "null") || a.highscores;
  ...
  var d = Class.create({
    ...
    draw: function (j) {
      ...
      for (q = 0, i = this.game.sparkles.all.length; q < i; ++q) {
        ...
        if (is.valid(m.image)) {
          ...
          j.drawImage(
            ...
            m.image * a.fruit.size,
            0,
            a.fruit.size,
            a.fruit.size,
            (m.pos.x - 1) * this.game.dw,
            (m.pos.y - 1) * this.game.dh,
            this.game.dw * 3,
            this.game.dh * 3
          );
        }
      ...
      j.drawImage(
        this.game.images.fruit,
        this.game.fruit.image * a.fruit.size,
        0,
        a.fruit.size,
        a.fruit.size,
      ...
      while ((k = k.next)) {
        this.drawPart(
          j,
          k,
          1 + Math.floor(this.nparts * (q++ / this.game.snake.length)),
          toInt(k.corner)
        );
      }
      for (q = 0, i = this.game.sparkles.all.length; q < i; q++) {
        ...
        j.fillStyle = m.color;
        j.globalAlpha = m.opacity;
        for (l = 0; l < m.particles.length; l++) {
          o = m.particles[l];
          j.fillRect(
            (m.pos.x + o.x + 0.5) * this.game.dw,
            (m.pos.y + o.y + 0.5) * this.game.dh,
            o.size * (this.game.dw / 2),
            o.size * (this.game.dh / 2)
          );
        }
      }
      ...
      this.parts = Game.renderToCanvas(
        ...
        function (i) {
          ...
          for (var k = 0; k < this.nparts; k++) {
            j = k / this.nparts;
            this.renderSegment(i, { x: 1 + k, y: 0 }, CORNER.NONE, j);
            this.renderSegment(i, { x: 1 + k, y: 1 }, CORNER.BL, j);
            this.renderSegment(i, { x: 1 + k, y: 2 }, CORNER.BR, j);
            this.renderSegment(i, { x: 1 + k, y: 3 }, CORNER.TL, j);
            this.renderSegment(i, { x: 1 + k, y: 4 }, CORNER.TR, j);
          }
          ...
          for (var k = 0; k < this.game.court.walls.length; k++) {
            this.renderWall(i, { x: k, y: 5 }, this.game.court.walls[k]);
          }
        }.bind(this)
      );
    },
